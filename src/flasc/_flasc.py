...
from ._hdbscan import (
    hdbscan_generic_reachability,
    hdbscan_generic_linkage,
    hdbscan_space_tree,
    hdbscan_space_tree_linkage,
    hdbscan_space_tree_core_dists_prims,
    hdbscan_extract_clusters,
)
from ._flasc_branches import (
    extract_cluster_points,
    extract_cluster_msts,
    compute_msts_in_cluster_generic,
    compute_msts_in_cluster_space_tree,
    compute_branch_linkage,
...
def flasc(
    ...
    **kwargs,
    ...
    the edges are clustered as if that centrality was a distance, progressively
    removing the 'center' of each cluster and seeing how many branches remain.

    Parameters
    ----------
    X : array of shape (n_samples, n_features), or \
        array of shape (n_samples, n_samples) A feature array, or array of
        distances between samples if ``metric='precomputed'``.

    override_cluster_labels : np.ndarray, optional (default=None)
        Override the FLASC clustering to specify your own grouping with a numpy
    ...
    min_branch_size : int, optional (default=None)
        The minimum number of samples in a group for that group to be considered
        a branch; groupings smaller than this size will seen as points falling
        out of a branch. Defaults to the min_cluster_size.

    min_samples : int, optional (default=None)
        The number of samples in a neighborhood for a point to be considered as
        a core point. This includes the point itself. Defaults to the
        min_cluster_size.
    ...
    alpha : float, optional (default=1.0)
        A distance scaling parameter as used in robust single linkage. See [2]_
        for more information.

    algorithm : str, optional (default='best')
        Exactly which algorithm to use; hdbscan has variants specialized for
        different characteristics of the data. By default this is set to
        ``best`` which chooses the "best" algorithm given the nature of the
        data. You can force other options if you believe you know better.
        Options are:
          * ``best``
          * ``generic``
          * ``prims_kdtree``
    ...
    approx_min_span_tree : bool, optional (default=True)
        ...
        clustering may be of marginally lower quality. If you are willing to
        sacrifice speed for correctness you may want to explore this; in general
        this should be left at the default True.

    max_cluster_size : int, optional (default=0)
        A limit to the size of clusters returned by the eom algorithm. Has no
        effect when using leaf clustering (where clusters are usually small
        regardless) and can also be overridden in rare cases by a high value for
        cluster_selection_epsilon. Note that this should not be used if we want
        to predict the cluster labels for new points in future (e.g. using
        approximate_predict), as the approximate_predict function is not aware
        of this argument.

    cluster_selection_method : str, optional (default='eom')
        The method used to select clusters from the condensed tree. The standard
        approach for FLASC is to use an Excess of Mass algorithm to find the
        most persistent clusters. Alternatively you can instead select the
        clusters at the leaves of the tree -- this provides the most fine
        grained and homogeneous clusters. Options are:
          * ``eom``
          * ``leaf``

    allow_single_cluster : bool, optional (default=False)
        By default FLASC will not produce a single cluster, setting this to
        True will override this and allow single cluster results in the case
        that you feel this is a valid result for your dataset. (default False)

    cluster_selection_epsilon: float, optional (default=0.0)
        A distance threshold. Clusters below this value will be merged. See [3]_
        for more information. Note that this should not be used if we want to
        predict the cluster labels for new points in future (e.g. using
        approximate_predict), as the approximate_predict function is not aware
        of this argument.

    cluster_selection_persistence: float, optional (default=0.0)
        A persistence threshold. Clusters with a persistence below this value
        will be merged. Note that this should not be used if we want to predict
        the cluster labels for new points in future (e.g. using
        approximate_predict), as the approximate_predict function is not aware
        of this argument.
    ...
    allow_single_branch : bool, optional (default=False)
        ...
        ``label_sides_as_branches`` FLASC requires at least 3 branches to exist
        in a cluster before they are incorporated in the final labelling.
    ...
    branch_detection_method : str, optional (default=``full``)
        ...
        sensitivity to noise: - ``core``: Contains the edges that connect each
        point to all other 
          points within a mutual reachability distance lower than or equal to
          the point's core distance. This is the cluster's subgraph of the k-NN
          graph over the entire data set (with k = ``min_samples``).
    ...
    branch_selection_method : str, optional (default='eom')
        ...
        The standard approach for FLASC is to use the ``eom`` approach. Options
        are:
          * ``eom``
          * ``leaf``

    branch_selection_epsilon: float, optional (default=0.0)
        A distance threshold. Branches below this value will be merged. See [3]_
        for more information. Note that this should not be used if we want to
        predict the cluster labels for new points in future (e.g. using
        approximate_predict), as the approximate_predict function is not aware
        of this argument.
    ...
    branch_selection_persistence: float, optional (default=0.0)
        A persistence threshold. Branches with a persistence below this value
        will be merged. Note that this should not be used if we want to predict
        the cluster labels for new points in future (e.g. using
        approximate_predict), as the approximate_predict function is not aware
        of this argument.
    ...
    num_jobs : int, optional (default=None)
        ...
        used. By default, the algorithm tries to estimate whether the given
        input is large enough for multi-processing to have a benefit. If so, 4
        processes are started, otherwise only the main process is used. When a
        ``num_jobs`` value is given, that number of jobs is used regardless of
        the input size.

    **kwargs : optional
        Additional arguments passed to :func:`~hdbscan.hdbscan` or the distance
        metric.

    Returns
    -------
    labels : np.ndarray, shape (n_samples, )
        Cluster+branch labels for each point. Noisy samples are given the label
        -1.
    ...
    cluster_persistence : array, shape  (n_clusters, )
        A score of how persistent each cluster is. A score of 1.0 represents a
        perfectly stable cluster that persists over all distance scales, while a
        score of 0.0 represents a perfectly ephemeral cluster. These scores
        gauge the relative coherence of the clusters output by the algorithm.
    ...
    branch_probabilities : np.ndarray, shape (n_samples, )
        Branch membership strengths for each point. Noisy samples are assigned
        0.
    ...
    condensed_tree : record array
        The condensed cluster hierarchy used to generate clusters. Not available
        when ``override_cluster_labels`` is used.

    single_linkage_tree : np.ndarray, shape (n_samples - 1, 4)
        The single linkage tree produced during clustering in scipy hierarchical
        clustering format (see
        http://docs.scipy.org/doc/scipy/reference/cluster.hierarchy.html). Not
        available when ``override_cluster_labels`` is used.
    ...
    cluster_approximation_graphs : tuple (n_clusters)
        ...
        The edges contained in the graphs depend on the
        ``branch_detection_method``: - ``core``: Contains the edges that connect
        each point to all other 
    ...
    branch_condensed_trees : tuple (n_clusters)
        A condensed branch hierarchy for each cluster produced during the branch
        detection step. Data points are numbered with in-cluster ids.

    branch_linkage_trees : tuple (n_clusters)
        A single linkage tree for each cluster produced during the branch
        detection step, in the scipy hierarchical clustering format. (see
        http://docs.scipy.org/doc/scipy/reference/cluster.hierarchy.html). Data
        points are numbered with in-cluster ids.
    ...
    .. [2] Chaudhuri, K., & Dasgupta, S. (2010). Rates of convergence for the
       cluster tree. In Advances in Neural Information Processing Systems (pp.
       343-351).
    ...
    # -- Override clustering
    def __1d_numpy_array_with_shape(length=X.shape[0], dtype=np.integer, **kwargs):
        for k, v in kwargs.items():
            if not (
    ...
    if override_cluster_labels is not None:
        if algorithm.startswith("boruvka"):
            raise ValueError("Cannot use Boruvka algorithm with overridden clusters.")
        ...
        def __check_override_labels(labels):
            ...
            if not np.all(np.diff(unique_values) == 1) or unique_values[0] != 0:
                raise ValueError("Overridden cluster labels are not consecutive!")

        __check_override_labels(override_cluster_labels)

        if override_cluster_probabilities is None:
            override_cluster_probabilities = np.ones(X.shape[0], dtype=np.double)
        else:
            __1d_numpy_array_with_shape(
                override_cluster_probabilities=override_cluster_probabilities,
                dtype=np.floating,
            )
            override_cluster_probabilities = override_cluster_probabilities.astype(
                np.double
            )
    else:
    ...
    # - Perform clustering
    # -- Compute single linkage hierarchy Declare variables as None
    ...
    core_distances = None
    neighbors = None  # not used with branch_detection_method != 'core'

    # Fill values depending on the cases
    if run_generic:
        if run_override:
            (reachability, core_distances, neighbors) = memory.cache(
                hdbscan_generic_reachability
            )(
                X,
                min_samples=min_samples,
            ...
            (
                ...
                core_distances,
                neighbors,
            ) = memory.cache(hdbscan_generic_linkage)(
                X,
                min_samples=min_samples,
                alpha=alpha,
                metric=metric,
        ...
        space_tree = memory.cache(hdbscan_space_tree)(
            X, metric=metric, algorithm=algorithm, leaf_size=leaf_size, **kwargs
        )
        if run_override:
            (core_distances, neighbors) = memory.cache(
                hdbscan_space_tree_core_dists_prims, ignore=["thread_pool"]
            ...
            (
                ...
                core_distances,
                neighbors,
            ) = memory.cache(hdbscan_space_tree_linkage, ignore=["thread_pool"])(
                space_tree,
                min_samples=min_samples,
                metric=metric,
                alpha=alpha,
    ...
    # -- Perform cluster segmentation Declare variables as None
    ...
    if run_override:
        num_clusters = len(np.unique(cluster_labels)) - int(-1 in cluster_labels)
    else:
        num_clusters = len(cluster_persistence)
    cluster_points = extract_cluster_points(cluster_labels, num_clusters)
    ...
    if not run_override:
        cluster_spanning_trees = extract_cluster_msts(
            min_spanning_tree, cluster_labels, num_clusters
        )
    elif run_generic:
        cluster_spanning_trees = memory.cache(
            compute_msts_in_cluster_generic, ignore=["thread_pool"]
        )(reachability, cluster_points, thread_pool)
    else:
        cluster_spanning_trees = memory.cache(
            compute_msts_in_cluster_space_tree, ignore=["thread_pool"]
        )(
            space_tree,
            core_distances,
            cluster_points,
