...
# Original Authors: Leland McInnes
...
from . import _hdbscan_dist_metrics as dist_metrics
from . cimport _hdbscan_dist_metrics as dist_metrics
...
cdef class BoruvkaUnionFind (object):
    ...
    def __init__(self, size):
        ...
        self._parent = self._parent_arr
        self._rank_arr = np.zeros(size, dtype=np.uint8)
        self._rank = self._rank_arr
...
cdef class KDTreeBoruvkaAlgorithm (object):
    ...
    thread_pool : joblib.parallel.Parallel
        A parallel pool to run core_distance querries.
    ...
    **kwargs :
    ...
    cdef bint approx_min_span_tree
    cdef np.intp_t min_samples
    cdef np.intp_t num_points
    cdef np.intp_t num_nodes
    cdef np.intp_t num_features

    cdef np.double_t[::1] core_distance
    cdef np.double_t[::1] bounds
    cdef np.intp_t[::1] component_of_point
    cdef np.intp_t[::1] component_of_node
    cdef np.intp_t[::1] candidate_neighbor
    cdef np.intp_t[::1] candidate_point
    cdef np.double_t[::1] candidate_distance
    cdef np.double_t[:, ::1] centroid_distances
    ...
    cdef public np.ndarray neighbour_arr
    ...
    def __init__(self, tree, min_samples=5, metric='euclidean', leaf_size=20,
                 alpha=1.0, approx_min_span_tree=False, thread_pool=None, 
                 **kwargs):
        ...
        self.bounds = self.bounds_arr
        self.component_of_point = self.component_of_point_arr
        self.component_of_node = self.component_of_node_arr
        self.candidate_neighbor = self.candidate_neighbor_arr
        self.candidate_point = self.candidate_point_arr
        self.candidate_distance = self.candidate_distance_arr
    ...
    cdef _compute_bounds(self):
        ...
        if self.thread_pool.n_jobs == 1:
            knn_dist, knn_indices = self.core_dist_tree.query(
                self.tree.data, k=self.min_samples + 1, 
                dualtree=True, breadth_first=True
            )
        else:
            datasets = []
            num_jobs = min(self.num_points, self.thread_pool.n_jobs)
            split_cnt = max(self.num_points // self.thread_pool.n_jobs, 1)
            for i in range(num_jobs):
                if i == num_jobs - 1:
                    datasets.append(self.tree.data.base[i*split_cnt:])
                else:
                    datasets.append(self.tree.data.base[i*split_cnt:(i+1)*split_cnt])
            knn_data = self.thread_pool(
                delayed(self.core_dist_tree.query)(
                    points, k=self.min_samples + 1, dualtree=True, breadth_first=True
                ) for points in datasets)
            knn_dist = np.vstack([x[0] for x in knn_data])
            knn_indices = np.vstack([x[1] for x in knn_data])
        self.neighbour_arr = knn_indices[:, 1:]
        self.core_distance_arr = knn_dist[:, self.min_samples].copy()
        self.core_distance = self.core_distance_arr
    ...
    cdef int update_components(self) except -1:
        ...
        if self.approx_min_span_tree:
            last_num_components = len(self.components)
            self.components = self.component_union_find.components()

            if len(self.components) == last_num_components:
                # Reset bounds
                for n in range(self.num_nodes):
                    self.bounds_arr[n] = <np.double_t> DBL_MAX
        else:
    ...
    cdef int dual_tree_traversal(self, np.intp_t node1,
        ...
        if node_dist < self.bounds[node1]:
            if (self.component_of_node[node1] ==
                ...
                for j in range(point_indices2.shape[0]):
                    ...
                    if component1 != component2:
                        ...
                        if self.alpha != 1.0:
                            ...
                            mr_dist = max(d, self.core_distance[p],
                                          self.core_distance[q])
...
cdef class BallTreeBoruvkaAlgorithm (object):
    ...
    **kwargs :
    ...
    def __init__(self, tree, min_samples=5, metric='euclidean',
                 alpha=1.0, leaf_size=20, approx_min_span_tree=False, thread_pool=None,
                 **kwargs):
        ...
        self.centroid_distances = self._centroid_distances_arr
    ...
    cdef _compute_bounds(self):
        ...
        if self.thread_pool.n_jobs == 1:
            ...
            datasets = []
            num_jobs = min(self.thread_pool.n_jobs, self.num_points)
            split_cnt = max(self.num_points // self.thread_pool.n_jobs, 1)
            for i in range(num_jobs):
                if i == num_jobs - 1:
                    datasets.append(self.tree.data.base[i*split_cnt:])
    ...
    cdef int dual_tree_traversal(self, np.intp_t node1,
        ...
        # otherwise we get to prune this branch and return early.
        if node_dist < self.bounds[node1]:
            if self.component_of_node[node1] == self.component_of_node[
                    node2] and self.component_of_node[node1] >= 0:
                return 0
        else:
            return 0
        ...
        if node1_info.is_leaf and node2_info.is_leaf:
            ...
            if new_bound < self.bounds[node1]:
                ...
                while node1 > 0:
                    ...
                    bound_max = max(self.bounds[left],
                                    self.bounds[right])
                    bound_min = min(self.bounds[left] + 2 *
